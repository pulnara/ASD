...
node* select (node*& L) {       // selects the biggest value
    node* biggestPred = NULL;
    ...
    while (tmp -> next != NULL) {
        if (tmp -> next -> val > maxVal) {
            biggestPred = tmp;
            maxVal = biggestPred -> next -> val;
        }
        tmp = tmp -> next;
    }

    if (biggestPred == NULL) {     // head contains the smallest value
        node* selected = L;
        L = L -> next;
        return selected;
    }
        ...
        node* selected = biggestPred -> next;
        biggestPred -> next = biggestPred -> next -> next;
        return selected;
    }
}
...
//    }
    ...
    while (i >= 0) {           // connect all of the small sorted parts
        while (T[i] -> next != NULL) T[i] = T[i] -> next;
        int j = i-1;
        while (j >= 0 and T[j] == NULL) j--;
        T[i] -> next = T[j];
        i = j;
    }
    node* tmp = L;
    for (int i = 0; i < n-1; i++) tmp = tmp -> next;
    tmp -> next = NULL;         // add 'NULL' at the end
//
//    for (int i = 0; i < n; i++) {
//        print(T[i]);
//        cout<<endl;
