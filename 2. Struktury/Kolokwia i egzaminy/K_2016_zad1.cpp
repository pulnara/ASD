...
struct SLNode {
    int val; // wartoœæ przechowywana w wêŸle
    int level; // poziom wêz³a
    SLNode** next; // level-elementowa tablica wskaŸników na nastêpniki
};

struct SkipList {
    SLNode* first; // wartownik przedni (pole value ma wartoœæ -oo)
    SLNode* last; // wartownik tylny (pole value ma wartoœæ +oo)
};
...
SkipList merge (SkipList A, SkipList B) {
    ...
    if (A.first -> next[0] -> val < B.first -> next[0] -> val) {
        node -> level = A.first -> next[0] -> level;
        node -> val = A.first -> next[0] -> val;
        ...
        node -> level = B.first -> next[0] -> level;
        node -> val = B.first -> next[0] -> val;
